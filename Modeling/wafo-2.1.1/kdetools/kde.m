...
if nv<d
  ...
  for ix=nv+1:d
	...
	otherwise,
     ...
     f.x{ix}=transpose(linspace(min(lo,amin(ix)),amax(ix)+xyzrange(ix)/4,41));
  end
end
...
if alpha>0,
  ...
  switch d
   ...
   case 2, lambda=interp2(Ai{:},lambda(ind),X{:},method);
   case 3, lambda=interp3(Ai{:},lambda(ind),X{:},method);
   otherwise ,  
     lambda =interpn(Ai{:},lambda(ind),X{:},method);
  end
  f.lambda=lambda;
else
