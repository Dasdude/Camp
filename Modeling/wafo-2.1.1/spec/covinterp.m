...
onedim=0;
Nin=length(R.R);
if prod(size(R.R))==Nin, % one-dim cvf
  onedim=1;
end
names=fieldnames(R);
if nargin<3
...
tin=getfield(R,vari);  %tin is either R.t or R.x
tin=tin(:)'; % make it row
if min(tin)>=0
  tin=[-tin(4:-1:2) tin];
end
...
for j=1:length(ind)
  ...
  Rout=setfield(Rout,names{ind(j)},Y);
end
Rout=setfield(Rout,vari,t);
