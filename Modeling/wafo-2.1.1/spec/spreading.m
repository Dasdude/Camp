...
if nargin<1|length(th)<2
  if (nargin>0) & (length(th)==1), Nt=abs(th);end
...
if Nt<40, warning('Number of angles is less than 40. Spreading too sparsely sampled!'), end
...
if strncmp(type,'cos2s',1),
  if isempty(wc),S=s;else,S = s(ones(Nt,1),:);end
...
if strncmp(type,'poisson',1),  
  if isempty(wc),X = r1; else, X = r1(ones(Nt,1),:);end
...
if strncmp(type,'sech2',1), 
  ...
  if isempty(wc),B=par; else,B = par(ones(Nt,1),:); NB = NB(ones(Nt,1),:); end
...
if strncmp(type,'mises',1),  
  if isempty(wc),K = par; else,K = par(ones(Nt,1),:);end
...
if strncmp(type,'box',1),
  if any(par>pi), error('BOX-CAR spreading: The A value must be less than pi'),end
  if isempty(wc),A=par; else, A = par(ones(Nt,1),:);end
  D.S=1./(2.*A).*(-A<=TH & TH<=A); 
...
switch lower(type(1))	 
  case {'s'}, % sech-2
     if def==0,
       ...
       def=1;data2=[2.61 2.28 0.52 1.3  -1.3  0.56 0.95 1.6]; 
     end
   case {'b'}, % box-car
     def=0;data2=pi/3;
   case {'p'}, % poisson
   ...
   otherwise,  % cos-2s 
     ...
     switch def
       case 0, data2=15;
       case 2, data2=[6.97 9.77 0.52 4.06 -2.52 0    1    inf];
       otherwise, %def =1
	 def=1; data2=[15   15   0.52 5    -2.5  0    1    inf];
     end
 end 
end
...
% box-char
    ...
    if mvnr>5.2,
      for ix=ix0,
	x(ix) = abs(fzero('sin(x)-x.*P1',x0(ix) ,optimset,r1(ix)));
      end
    else
      for ix=ix0,
	x(ix) = abs(fzero('sin(x)-x.*P1',x0(ix) ,[],[],r1(ix)));
      end
    end
