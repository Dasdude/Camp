...
if all(freq>0) % for .type='k2d', negative frequencies are allowed
  disp('Spectrum does not start at zero frequency/wave number.')
  error('Correct it with specinterp, for example.')
%   disp('trying to fix it, but numerical problems may occur')
%   wfix=(0:dw:(ws-dw)).';
%   nfix=length(wfix);
%   specn=[zeros(nfix,1);specn];
%   w=[wfix;w];
%   n=n+nfix;
end
if any(abs(diff(diff(freq)))>1.0e-8)
  disp('Not equidistant frequencies/wave numbers in spectrum.')
  error('Correct it with specinterp, for example.')
end
...
if prod(size(S.S))~=length(S.S)&(nargin>4 & Nx==0)&(nargin>5 & Ny==0)
  S = spec2spec(S,'freq');
