...
%  wspecplot(S,2), hold on
%  wspecplot(S,3,'g')  % Same as previous fig. due to frequency independent spreading
%  wspecplot(S2,2,'r') % Not the same as previous figs. due to frequency dependent spreading
%  wspecplot(S2,3,'m')
...
if abs(thmax-thmin)<3*pi, %Radians given. Want xticks given as fractions  of pi
  ...
  if xy=='x'	
    if dim<3, axis([thmin,thmax 0 inf ]),else,axis([thmin,thmax 0 inf 0 inf]),end
  else
    if dim<3, axis([0 inf thmin,thmax ]),else,axis([0 inf thmin,thmax 0 inf]),end
  end
