...
foundSourceCode = 0;
while ((~foundSourceCode) & (feof(fid)==0)), 
...
if (~any(isletter(tline)) | length(tline)<2|...
  ...
  warning(['No help header is found in '  filename])
  return
end
...
if strcmpi(fn,filename)
  ...
  warning(['Wrong H1LINE-style: Filename differ from first token in' ...
	   ' H1LINE.'])
  fn = filename;
