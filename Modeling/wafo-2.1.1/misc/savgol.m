...
if (np<nl+nr+1)
  error(sprintf('np must be larger than nl+nr = %d or nl+nr less than np = %d',nr+nl,np))
end
if nl<0, 
  warning('nl must be positive or zero')
  nl = max(nl,0);
end
if nr<0, 
  warning('nr must be positive or zero')
  nr = max(nr,0);
end
if ld>m 
  error(sprintf('m must be larger than ld = %d ',ld))
end
if nl+nr<m,
  error(sprintf('m must be smaller than nl+nr = %d',nl+nr))
end
