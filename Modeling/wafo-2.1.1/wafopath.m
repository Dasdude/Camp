...
if strcmp(part,'tools')
  p = {[proot 'exec' filesep lower(computer)]}; % Search for mex-files first
						
  if 1, % new call 
    w=dir(proot); inddir=find(cat(1,w.isdir));
    wdir={w(inddir).name};
    ...
    p(end+1,1) = {[proot 'onedim']};
    p(end+1,1) = {[proot 'multidim']};
    p(end+1,1) = {[proot 'spec']};
    p(end+1,1) = {[proot 'trgauss']};
    p(end+1,1) = {[proot 'wavemodels']};
    p(end+1,1) = {[proot 'wsim']};
    p(end+1,1) = {[proot 'wstats']};
    p(end+1,1) = {[proot 'kdetools']};
    p(end+1,1) = {[proot 'misc']};
    p(end+1,1) = {[proot 'cycles']};
    p(end+1,1) = {[proot 'markov']};
    %p(end+1,1) = {[proot 'smarkov']};
    p(end+1,1) = {[proot 'damage']};
    % p(end+1,1) = {[proot 'fplot']};
  end
