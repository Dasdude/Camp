...
if nargin<3  
   display('too few inputs parameters, stop')
   break
end
...
if ntc < 2
  display('the reference level out of range, stop')
    break
    end
if ntc > (param(3)-1)
  display('the reference level out of range, stop')
...
if ntc > n-1
  display('index for mean-level out of range, stop')
    break
    end
