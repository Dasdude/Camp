...
%  - updated call empdistr(X,-inf,[],monitor) to  empdistr(X,[],monitor)
%    due to new calling syntax for empdistr
...
if strcmpi(lower(def(1:3)),'lin') & nargout<=2, return,end
...
if multip==1,
  for ix=1:(m-1),
    ...
    if (lower(def(1))=='m')
      Fx  = empdistr(xx(:,[ ix+1]),[],plotflag==2);
      if plotflag==2
	pause(opt.delay)
      end
    end
    switch lower(def(1:3))
      case {'her'},
	ga1 = wskewness(xx(:,ix+1));
	ga2 = wkurtosis(xx(:,ix+1))-3;
	up  = min(4*(4*ga1/3).^2,12);
	lo  = ga1^2*3/2;
	kurt = min(up,max(ga2,lo))+3;
	phat = [sa(ix), ga1, kurt,ma(ix) ];
	[g(:,2*ix-1:2*ix), test(ix)] = hermitetr(g(:,2*ix-1) ,phat);
	g2=[];
      case {'och'},
	phat = [ sa(ix) wskewness(xx(:,ix+1)) ma(ix)];
  ...
  if (lower(def(1))=='m')
    Fx  = empdistr(xx(n+1:end),[],plotflag==2);
    if plotflag==2
      pause(opt.delay)
    end
  end
  switch lower(def(1:3))
    case {'her'},
      ga1 = wskewness(xx(n+1:end));
      ga2 = wkurtosis(xx(n+1:end))-3;
      ...
      g2=[];
    case {'och'}
      phat = [sa wskewness(xx(n+1:end)) ma];
