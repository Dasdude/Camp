function [f] = spec2tccpdf(spec,utc,def,paramt,h1,h2,nit,speed,bound,plotflag)
...
if abs(defnr)<1.5
   ...
   dos([ wafoexepath 'sp2tccpdf51.exe']); 
end
...
if defnr==-2   
  ...
  flo=reshape(tmp(:,2),4,length(t));
  ff=fup(4,:)+fup(1,:)-flo(2,:)-flo(3,:);
  gg=max(flo(4,:)+flo(1,:)-fup(2,:)-fup(3,:),0.); 
  f.f=[ff', gg']';
  %f.f=[fup;flo];
end
if defnr==-1   
