...
%  gder(:,1) = g(:,1);  
%  plot(g(:,1),[g(:,2),gder(:,2)])
%  plot(g(:,1),wnormpdf(g(:,2)).*gder(:,2),g(:,1),wnormpdf(g(:,1)))
...
if N>0
  ...
  if hn^N<sqrt(eps)
    ...
    warning('The sampling of the transformation may be too small.')
  end
...
%	      8*fder(ix-1,2)+fder(ix-2,2))./(12*hn);
  ...
  if N>1
	...
	if N>4
	  warning(['Transformation of derivatives of order>4 not supported' ...
		' in tranproc.'])
	end
      end
    end
