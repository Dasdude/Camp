...
defaultSpeed   = 7;
defaultoptions = initoptions(defaultSpeed);
...
if callFortran, % call fortran program
  dens = callSp2thpdfexe(R,dt,u,defnr,Nstart,hg,options);
  err  = repmat(nan,size(dens));
else
  [dens,err,options] = cov2thpdf(R,dt,u,defnr,Nstart,hg,options);  
end
...
if abs(defnr)>1, 
  try
   [f.cl,f.pl] = qlevels(f.f,[10, 30, 50, 70, 90, 95, 99],f.x{1},f.x{2});
    ...
    warning('Unable to calculate contourlevels')
  end
end
...
function dens = callSp2thpdfexe(R,dt,u,defnr,Nstart,hg,options)
  ...
  if ~isempty(hg)
   ...
   dos([ wafoexepath 'sp2thpdf70.exe']);
   ...
   function  [pdf,err,options] = cov2thpdf(R,dt,u,def,Nstart,h,options)
...
%opt0 =  {SCIS,XcScale,ABSEPS,RELEPS,COVEPS,MAXPTS,MINPTS};
  if (abs(def)<2) 
    ...
    for Ntd=Nstart:Ntime,
      ...
      indI(4) = Ntd;
      Ntdc    = Ntd+Nc;
      [pdf(Ntd),err(Ntd)] = rind(BIG(1:Ntdc,1:Ntdc),ex(1:Ntdc),...
	...
	indI(2) = ts-2;
	indI(3) = ts-1;          
	[fxind,err0] = rind(BIG(1:Ntdc,1:Ntdc),ex(1:Ntdc),a_lo, ...
