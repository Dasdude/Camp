function f = spec2mmtpdf(spec,utc,def,paramt,paramu,options,bound) 
...
if callFortran, % call fortran
   ftmp = callSp2mmtpdfexe(R,dt,u,defnr,Nstart,hg,bound,options);
   err = repmat(nan,size(ftmp));
else
  [ftmp,err,options] = cov2mmtpdf(R,dt,u,defnr,Nstart,hg,options);   
end
...
if bound<0.5
 if Nx>2  % amplitude distributions wanted
  ...
  % The lines below must be fixed: (it is not correct yet
  %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  if Nx>2
    ftmp_up=reshape(ftmp(:,1),Nx,Nx); 
    ftmp_lo=reshape(ftmp(:,2),Nx,Nx); 
    for i=1:Nx 
      ftmp_up(:,i)=dh*dh*der(i)*ftmp_up(:,i).*der;%*sqrt(-R(1,6)/R(1,4))/2/pi; 
      ftmp_lo(:,i)=dh*dh*der(i)*ftmp_lo(:,i).*der;%sqrt(-R(1,6)/R(1,4))/2/pi;
    end 
    if (defnr==0)
      f.f{1}=fliplr(mctp2rfc(fliplr(ftmp_up)));
      f.f{2}=fliplr(mctp2rfc(fliplr(ftmp_lo)));
    end
    if (defnr==1)
      f.f{1}=ftmp_up; 
      f.f{2}=ftmp_lo;
    ...
    if (defnr==-1)
      f.f{1}=fliplr(mctp2tc(fliplr(ftmp_up)));
      f.f{2}=fliplr(mctp2tc(fliplr(ftmp_lo)));
    end
    f.x{2}=h; 
  else
    size(ftmp)
...
function dens  = callSp2mmtpdfexe(R,dt,u,defnr,Nstart,hg,bound,options)
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if bound<0.5 
     dos([ wafoexepath 'sp2mmt70.exe']); %compiled sp2mmtpdf.f with rind70.f 
  else 
     dos([ wafoexepath 'sp2mM51.exe']); %compiled spec2tthpdf1.f with rind49.f 
  end 
  dens = load('dens.out');
  return
