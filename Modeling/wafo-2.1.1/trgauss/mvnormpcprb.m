function [val, err,ier,extime]= mvnormpcprb(rho,a,b,tol,useSimpson,useBreakPoints)
...
verstr = version;
ind = find(verstr=='.');
versionNumber = str2num(verstr(1:ind(2)-1));
t1 = clock;
% Call fortran implementation
if versionNumber<=5.3
  [val,err,ier] = mvnprodcorrprbmex53(rho,a,b,tol,reltol,...
				      useBreakPoints,useSimpson);
else
  [val,err,ier] = mvnprodcorrprbmex(rho,a,b,tol,reltol,...
				    useBreakPoints,useSimpson);
end
extime = etime(clock,t1);
if (ier>0)
  warning(sprintf('Abnormal termination ier = %d',ier))
  disp(getErrorMessage(ier))
end
return
...
ind = find(rho==0);
if any(ind)
  val0    = prod(wnormcdf(b(ind))-wnormcdf(a(ind)));
  a(ind)  = [];
  b(ind)  = [];
  rho(ind)= [];
  den(ind) = [];
...
xCutOff    = abs(max(wnorminv(0.05*reltol),-37));
xlo   = -xCutOff; %abs(max(wnorminv(0.0001*reltol),-37))
xup   = -xlo;
...
if any(ind)
  ...
  den(ind) = [];
  if length(a)==0
    val = (wnormcdf(xup)-wnormcdf(xlo))*val0;
    err = sqrt(eps);
    return
  end
end
