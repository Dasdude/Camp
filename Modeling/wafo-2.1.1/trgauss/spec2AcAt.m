function [f] = spec2AcAt(spec,utc,def,paramtc,paramtt,paramt,h1,h2,nit,speed,bound)
...
if strcmp('l',lower(def(1)))
  FAc=spec2Acdf(spec,utc,'Lc',paramtc,h1,nit0,speed,bound); 
elseif strcmp('t',lower(def(1)))
  FAc=spec2Acdf(spec,utc,'Tc',paramtc,h1,nit0,speed,bound);
else
  error('Unknown def')
end
pdfplot(FAc);
...
subplot(222)
if strcmp('l',lower(def(1)))
  FAt=spec2Acdf(spec,utc,'Lt',paramtc,h1,nit0,speed,bound); 
elseif strcmp('t',lower(def(1)))
  FAt=spec2Acdf(spec,utc,'Tt',paramtc,h1,nit0,speed,bound);
else
  error('Unknown def')
end
...
if bound==1
   gg1=reshape(tmp(:,2),Nx1*Nx2,length(t)); 
   fff=zeros(Nx1,Nx2);
   ggg=fff;
   for j=1:Nx1
    for i=1:Nx2
     ii1=i+(j-1)*Nx2;
     fff(j,i)=max(dt*sum(ff1(ii1,:))+1.-FAc.f(2,j)-FAt.f(2,i),0.);
   ...
   for j=1:Nx1
    for i=1:Nx2
     ii1=i+(j-1)*Nx2;
     ggg(j,i)=max(dt*sum(gg1(ii1,:))+1.-FAc.f(1,j)-FAt.f(1,i),0.);
     ggg(j,i)=min(ggg(j,i),1.);
    end
   end
   subplot(223)
   contour(h1,h2,fff',[1, 0.9, 0.75, 0.5, 0.25, 0.1, 0.05])
   subplot(224) 
   contour(h1,h2,ggg',[1, 0.9, 0.75, 0.5, 0.25, 0.1, 0.05])
   f.f{1}=fff';
   f.f{2}=ggg';
else
   subplot(212) 
