function [fxind, err,exTime,options] = rind(BIG,Ex,Blo,Bup,indI,xc,Nt,varargin)
...
%
% See also  mvnormprb, mvnormpcprb, rindoptset
...
if isempty(options.xcutoff)
  ...
  options.xcutoff = max(min(abs(wnorminv(truncError/(Nc1c2*2))),8.5),1.2);
...
%  xCutOff = max(min(abs(wnorminv(max(RELEPS,ABSEPS)*10^(-1+0*(Nt>10)))),7),1.2);
%end
...
if options.method>0
  ...
  try
    if options.method>10
      ...
      % rindalan24 with removal of infis and stricter c1c2
      [fxind,err] = mexrindalan24(BIG,Ex,indI,Blo,Bup,INFIN,xc,Nt,opt0{[1:10,14]});
    end
  catch	  
    error('Compile mexrindalan24 again')
  end 
  ...
  try % mexcompiled function
    ...
    fxind = mexrind71(BIG,Ex,xc,Nt,NIT1,speed,indI,Blo,Bup,seed,xcscale);
