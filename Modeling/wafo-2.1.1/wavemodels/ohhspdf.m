...
if multipleSeaStates
  [errorcode, Scf,Hd,Hm0] = comnsize(Scf,Hd,Hm0);
else
...
%ch   = spec2char(ohspec2(w,[Hm0,def]),{'Tm02','eps2'});
%Tm02 = ch(1);
%Vrms = 2*Hm0/Tm02;
Hm00  = OHHSPAR.Hm0;
...
if nargout>3,
  ...
  fA.note = sprintf('%s \n',...
		    ...
		    'h=Hd/Hrms and Hm0 for the ohspec2 spectrum, ',...
...
if norm==0
  f.f = repmat(Fh/Hrms,[1 length(v)]).*wweibpdf(V1,A1,B1)/Vrms;
  f.x = {v,h};
  f.norm=0;
  f.labx={'Scf', 'Hd [m]'};
else
  f.f = repmat(Fh,[1 length(v)]).*wweibpdf(V1,A1,B1);
  f.x = {v/Vrms,h/Hrms};
  f.labx={'Scf', 'Hd'};
  f.norm = 1;
end
