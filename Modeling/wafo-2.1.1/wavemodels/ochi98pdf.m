...
%  plot(x,ochi98pdf(x,a1,a2),x,wraylpdf(x,sqrt(a1^2+a2^2)) 
%
% See also  wnormcdf, wraylpdf
...
if any(k),
 ...
 y(k)=2*xk./(r1+r2).^2.*(r1.*exp(-xk.^2./r1)+   r2.*exp(-xk.^2./r2)+...
    ...
    .*(2*xk.^2./(r1+r2)-1).*(wnormcdf(sqrt(2*r2./r1./(r1+r2)).*xk )    - wnormcdf(-sqrt(2*r1./r2./(r1+r2)).*xk ) ));
  end


k=find(a > 0 & (b==0 | a==b));
if any(k),
  y(k)=wraylpdf(x(k),a(k)/sqrt(2));
end
k=find(a == 0 & b>0);
if any(k),
  y(k)=wraylpdf(x(k),b(k)/sqrt(2));
end
