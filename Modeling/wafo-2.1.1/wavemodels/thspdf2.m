...
if nargin<5 
   ...
   f.f = wtweibpdf(H1,A0,B0,C0).*wgampdf(V1,A1,B1)/Vrms/Hrms;
 % f.f = repmat(Fh/Hrms,[1 length(v)]).*wgampdf(V1,A1,B1)/Vrms;
  f.x = {v,h};
  f.norm=0;
  f.labx={'Scf', 'Hd [m]'};
else
  f.f = wtweibpdf(H1,A0,B0,C0).*wgampdf(V1,A1,B1);
  %f.f = repmat(Fh,[1 length(v)]).*wgampdf(V1,A1,B1);
  f.x = {v/Vrms,h/Hrms};
  f.labx={'Scf', 'Hd'};
  f.norm = 1;
end
