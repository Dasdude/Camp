...
%   contour(x,x,f)
%   
% See also  wweibpdf, wlognpdf, wggampdf
...
if any(k0)
  if condon==5,
     p(k0) = wlogncdf(t(k0),myTp(k0),sTp(k0));
   elseif condon ==10
     p(k0) = 1;
   else
    p(k0) = wlognpdf(t(k0),myTp(k0),sTp(k0));
  end
end
...
if condon==0 | condon==5 | condon ==10
  ...
  if any(k1)
    if def<=1,
      ...
      p(k1)=p(k1).*wweibpdf(h(k1)-C3,C1,C2);
    else
      p(k1)=p(k1).*wggampdf(h(k1),C1,C2,C3); 
    end
  end
end
