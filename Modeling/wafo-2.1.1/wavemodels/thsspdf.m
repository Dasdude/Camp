...
multipleSeaStates = any(prod(size(Hm0))>1|prod(size(Tp))>1);
if multipleSeaStates
  [errorcode, Scf,Hd,Hm0,Tp] = comnsize(Scf,Hd,Hm0,Tp);
else
  [errorcode, Scf,Hd] = comnsize(Scf,Hd);
end
if errorcode > 0,
...
if useWeibull
  switch condon,
   case 0, % regular pdf is returned 
    f = wtweibpdf(h,A0,B0,C0).*wgampdf(v,A1,B1);
  ...
  switch condon,
   case 0, % regular pdf is returned 
    f = wggampdf(h,A0,B0,C0).*wgampdf(v,A1,B1);
   case 1, %pdf conditioned on x1 ie. p(x2|x1) 
    f = wgampdf(v,A1,B1);
   case 3, % secret option  used by XXstat: returns x2*p(x2|x1) 
    f = v.*wgampdf(v,A1,B1);
   case 4, % secret option  used by XXstat: returns x2.^2*p(x2|x1) 
    f = v.^2.*wgampdf(v,A1,B1);
   case 5, % p(h)*P(V|h) is returned special case used by thscdf
    f = wggampdf(h,A0,B0,C0).*wgamcdf(v,A1,B1);
   case 6, % P(V|h) is returned special case used by thscdf
    f = wgamcdf(v,A1,B1);
   case 7,% p(h)*(1-P(V|h)) is returned special case used by thscdf
    f = wggampdf(h,A0,B0,C0).*(1-wgamcdf(v,A1,B1));
   otherwise error('unknown option')
  end
end
if multipleSeaStates
...
if nargout>3,
  ...
  fA(1).title = 'wggampdf parameter A';
  fA(2).title = 'wggampdf parameter B';
  fA(3).title = 'wggampdf parameter C';
  
  txt1 = ['The Wggampdf distribution Parameter '];
...
if nargout>4,
  ...
  txt11 = 'The conditonal Wgampdf distribution Parameter ';
  txt22 = [' for Scf given h=Hd/Hrms in space as function of Tp' ...
	' and Hm0 for the Torsethaugen spectrum'];
  fB(1).title = 'wgampdf parameter A';
  fB(2).title = 'wgampdf parameter B';
  
  fB(1).note = [txt11,'A',txt22];
  fB(2).note = [txt11,'B',txt22];
  %fB(2).note = ['The conditonal Wggampdf distribution Parameter B(h)/Hrms' ...
