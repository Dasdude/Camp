...
if (nargin <4)| isempty(tol), 
  tol=1e-3;%sqrt(eps);%relative accuracy of the estimates
end
if nargin <  3, 
    error('Requires at least three input arguments.'); 
end

[errorcode x a b] = comnsize(x,a,b.^2);
...
if any(k),
  ...
  if 0,all((ak==ak(1)).*(bk==bk(1))),
    ...
    gn=2;
    [y(k), tol(k)]= gaussq('tay81pdf',0,xk,tol,trace,gn,ak,bk,tol);
    %[y(k), tol(k)]= asimpson('traylpdf',0,xk,tol,trace,ak,bk);
  end
end
if 0
...
if any(k4), % special case b==inf -> rayleigh distribution
 y(k4) = wraylcdf(x(k4),a(k4)/sqrt(2));
end 
