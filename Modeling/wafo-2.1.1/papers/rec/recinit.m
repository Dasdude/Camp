...
if RECFIGNUM>6,				 
  ...
  if isempty(phat)
    ...
    phat=dist2dfit(V/Vrms,H/Hrms,{'weibull','rayleigh'},[res,noverlap]);
  end
  if isempty(sphat)
    sphat=dist2dsmfun2(phat,linspace(0,3.5),[CSMA,CSMB ],[1 1 ]); % smooth the estimated parameter values
  end

  % Calculate the theoretical density of fitted model
  if isempty(ft2)				    
    ft2=dist2dpdf2(linspace(0,4),linspace(0,4),sphat);
    ft2.labx{1}='v';
    ft2.labx{2}='h';
  end
end
