...
if Np>0
  ...
  Nk = length(k);
  if Nk>0
    if Nk>3,      warning('More than 3 strings are not allowed'),    end
    for ix = k
      switch lower(P{ix})
	case {'f','w'},                                 ftype  = P{ix};
	case {'cov','pmem','mem','psd','psdo','pburg'}, method = P{ix};
	case {'mean','ma','linear','none'},              dflag = P{ix};
	otherwise,                    warning(['Unknown option:' P{ix}])
      end
    end
    Np = Np-Nk;
...
max_L     = 300; % maximum lag if L is undetermined
changed_L = 0;
if isempty(L)
...
switch lower(method)
  case {'psd','psdo'} % from signal toolbox
    noverlap   = 0;
    if method(end)=='o', noverlap   = floor(L/2);end
    S.noverlap = noverlap;
    [Rper Sc f]=psd(yy(:,2),nfft,1/dT,win,noverlap,p,dflag);
     for ix=3:m
  ...
  if (  ~isempty(p) ),
    ...
    S.CI = [v/wchi2inv( 1-alpha/2 ,v) v/wchi2inv( alpha/2 ,v)];
    S.p  = p;
  end
end
...
if any(ind)
  ...
  warning('negative spectral estimates')
end
