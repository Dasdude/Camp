...
for ix = k, % looping over non-zero values of S(f) only
  ...
  if vstr>5.2, % matlab version 5.3 or higher
  ...
  else  % matlab version 5.2 or higher
    %La = fsolve('memfun',La,[],[],thetai,Pj(ones(nt,1),:)-qj,qj,M2);
    La = fmins('memfun',La,[],[],thetai,Pj(ones(nt,1),:)-qj,qj(:,1:M2),M2);
    %La = fminu('memfun',La,[],[],thetai,Pj(ones(nt,1),:)-qj,qj,M2)
    %La = fmins(eqstr,La,[],[],thetai,Pj(ones(nt,1),:)-qj,qj,M2)
  end
