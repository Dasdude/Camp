function [Sd,D,Sw,Fcof,Gwt,Sxy,Sxy1] = dat2dspec2(xn,pos,h,nfft,nt,method,varargin)
...
% TODO % Remove dependence on psd and csd in signal toolbox.
...
if nargin>6,  opt  = specoptset(opt,varargin{:}); end
...
[n m]= size(xx);
if n<m, b=m;m=n;n=b; xx=xx'; end

if n<4,   error('The vector must have more than 4 elements!'),end
...
par = [];g=[];rho=[];bet=[];igam=[];thx=[];thy=[]; % use default values in tran
if nargout==0, plotflag = 1;  else,    plotflag = 0;  end
...
if nt<30,  warning(' # of angles are low =>  spectral density may be inaccurate'), end
if nharm>=nt, 
  nharm = nt-1;
  disp('Nharm must be less than Nt')
end
...
case 'ma',     xx(:,2:m) = detrendma(xx(:,2:m),2*nfft);% remove moving average
               dflag     = 'linear'; % must change to a valid option for psd and csd
end
