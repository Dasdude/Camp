function [phat, cov,pci]=wggamfit(data1, plotflag,chat0);
...
ld   = log(data);
if nargin>2 & ~isempty(chat0) & isfinite(chat0)
  chat = chat0;
  chatDeterministic = 1;
else
  ...
  if 1, 
    ...
    x  = [linspace(max(eps,start/100),start,25) ...
  ...
  def=1; % What is def? See 'wggambfun' 
  if (def==1)
    if (mvrs > 5.2),
    ...
    start = [ahat,bhat,chat]
    if  (mvrs  >  5.2)
      phat = fminsearch('wggambfit',start,optimset,data,F,def);
    else
      phat = fmins('wggambfit',start,sqrt(eps),[],data,F,def);
    end
    chat = phat(3);
  end
end
if 0,
  phatAB = wgamfit(data.^chat,0);
  ahat = phatAB(1);
  bhat = phatAB(2).^(1/chat);
else
  dc   = data.^chat;
  ahat = -(chat*(mean(ld)-sum(dc.*ld)/sum(dc)))^(-1);
  bhat = (mean(dc)/ahat).^(1/chat);
