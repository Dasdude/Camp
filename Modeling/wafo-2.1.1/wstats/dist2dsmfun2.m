function sphat=dist2dsmfun2(phat,h2,csm,lin,visual,f0)
...
if useSTATS & ~isempty(phat.stats1) & strncmpi(phat.dist{1},'gamma',2)
   ...
   linA = 1;
   linB = 1;
   if (~isempty(csm) & length(csm)>0), CSMA = csm(1); end
   if (~isempty(csm) & length(csm)>1), CSMB = csm(2); end
   if (~isempty(lin) & length(lin)>0), linA = lin(1); end
  ...
  [sphat.x{1}(:,2), Bv , Cv]=dist2dsmfun(phat,h2,csm,lin);
end
  if ~isempty(Bv)
    sphat.x{1}(:,3)=Bv;  
  end
...
for ix = 1:10
  figure(fig0+2)
  dist2dparamplot(phat,sphat,1)
  if (ix>1),
    hold on
    dist2dparamplot(phat,sphat0,1)
    hold off
  end
  [x,y]=graphicinput;
  ...
  if ~isempty(Bv)
    figure(fig0+3)
    dist2dparamplot(phat,sphat,2)
    if (ix>1),
      hold on
      dist2dparamplot(phat,sphat0,2)
      hold off
    end
    [x,y]=graphicinput;
  ...
  if ~isempty(Cv) & any(Cv~=0),
    ...
    figure(fig0+4)
    dist2dparamplot(phat,sphat,col)
    if (ix>1),
      hold on
      dist2dparamplot(phat,sphat0,col)
      hold off
    end
    [x,y]=graphicinput;
