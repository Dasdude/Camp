function [ax11, h11, h22  ]=dist2dstatplot(V,H,varargin) 
...
if 1,%new call
  ...
  if Np>0
    ...
    if any(k)
      Nk = length(k);
      if Nk>4,  warning('More than 4 strings are not allowed'),    end
      iy = 1;
      for ix=k      
	sym{iy} = P{ix};
	iy=iy+1;
  ...
  %function [ax11, h11, h22  ]=dist2dstatplot(V,H,phat,condon ,res,csm,lin)
...
if ~isempty(phat), % theoretical distribution not given
  ...
  [M1 V1] = dist2dstat(phat,condon,cvar,csm,lin);
end
