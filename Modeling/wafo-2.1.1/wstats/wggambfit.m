...
switch def
  case 1,
    ...
    h   = 1e-6;
    h1  = .5*1e+6;
    if ((a<=0) | isnan(a)), % Avoid error with gammaln for a<0 pab 27.01.2001
      l = NaN;
    elseif (a<=h)
      l = log(length(data)*a) - (gammaln(a+h)-gammaln(a))/h + b*mld ....
	  -log(sdb);
    else
      l = log(length(data)*a) - (gammaln(a+h)-gammaln(a-h))/(2*h) + b*mld ....
	  -log(sdb);
    end
