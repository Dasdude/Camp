...
if strcmp(method,'gpd')
    [gpd_max,cov_max] = wgpdfit(y_max,'ml',0);
    [gpd_min,cov_min] = wgpdfit(y_min,'ml',0);
    ci_k_max = [gpd_max(1)-2*sqrt(cov_max(1)) gpd_max(1)+2*sqrt(cov_max(1))]
    ci_k_min = [gpd_min(1)-2*sqrt(cov_min(1)) gpd_min(1)+2*sqrt(cov_min(1))]
end
...
if plotflag>0
    ...
    subplot(2,2,2), if length(y_max)>0, wexpplot(y_max), end
    ...
    subplot(2,2,4), if length(y_min)>0, wexpplot(y_min), end
    drawnow
    ...
    for k = 1:N
        ...
        if length(y_max)>0
            if strcmp(method,'exp')
                yr_max = wexprnd(a_max,length(y_max),1);
            else
                yr_max = wgpdrnd(gpd_max(1),gpd_max(2),0,length(y_max),1);
            end
        else
            yr_max=[];
        end
        ...
        if length(y_min)>0
            if strcmp(method,'exp')
                yr_min = wexprnd(a_min,length(y_min),1); 
            else
                yr_min = wgpdrnd(gpd_min(1),gpd_min(2),0,length(y_min),1);
            end
        else
            yr_min=[];
        end
