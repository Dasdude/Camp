    ...
    if strcmp(methodEst,'ml')
      ...
      if ~isempty(I) % If ML-estimator exists, then use it! 
        ...
        if exist('optimset') >= 2 % Function 'optimset' exists ?
          ...
          x_ML = fzero('wgpdfit_mld',x_start,optimset('disp','off'),dNN);
        else 
          % For Matlab 5.2 and lower ???
          x_ML = fzero('wgpdfit_mld',x_start,[],[],dNN);
        end
        [f,shape,scale] = wgpdfit_mld(x_ML,dNN); % Estimates k_ML and s_ML
      ...
      if strcmp(methodEst,'ml0')
        [parms,cov] = wgpdfit(x,'ml',0);
      else
        [parms,cov] = wgpdfit(x,methodEst,0);
      end
    end
